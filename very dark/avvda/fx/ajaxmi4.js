...
for (i = 0; i <= idani.length ; i++) {
	if (idani[i] == document.getElementById(id))
		{
		switch(idani[i].style.visibility) {
					case "hidden":
					idani[i].style.display = 'none'
					break;
					case "visible":
					idani[i].style.display = 'block'
					break;
					}
		switch(idani[i].style.display) {
		case 'none' :
		idani[i].style.visibility = 'hidden';
		break;
		case 'block' :
		idani[i].style.visibility = 'visible';
		break;
					}
